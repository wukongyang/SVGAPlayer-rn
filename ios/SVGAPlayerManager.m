        ...
        [parser parseWithURL:[NSURL URLWithString:source]
             completionBlock:^(SVGAVideoEntity *_Nullable videoItem) {
               [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                 ...
                 // 如果初始化状态是play才执行播放
                if ([[self currentState] isEqualToString:@"start"]) {
                        [self startAnimation];
                 }
               }];
             }
...
// RCT_EXPORT_METHOD(setImage:(nonnull NSNumber *)reactTag details:(NSDictionary *)details){
...
//             if ([view isKindOfClass:SVGAPlayer.class]) {
//                 //此处获取到了系统的MKMapView组件，可以调用MKMapView的内置方法
//                 // { code }
//                 [view setImageWithURL:[NSURL URLWithString: @"https://github.com/PonyCui/resources/blob/master/svga_replace_avatar.png?raw=true"] forKey:@"99"];
//             }
//         }];
