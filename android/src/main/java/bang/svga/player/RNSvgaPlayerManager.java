...
public class RNSvgaPlayerManager extends SimpleViewManager<RCTSVGAImageView> {
            ...
            try {
                new SVGAParser(context).parse(new URL(source), new SVGAParser.ParseCompletion() {
                    ...
                    public void onComplete(SVGAVideoEntity videoItem) {
                        ...
                        view.onLoadingEnd();
                        if(view.currentState.equals("start")){
                             view.startAnimation();
                        }
                    }
    ...
    public Map<String, Integer> getCommandsMap() {
        return MapBuilder.of("setImage", SET_IMAGE,"test",TEST);
    }
    /**
    ...
    public void receiveCommand(@NonNull RCTSVGAImageView root, String commandId, @Nullable ReadableArray args) {
        System.out.print("========receiveCommand===++++==========");
        int commandIdInt = Integer.parseInt(commandId);
        switch (commandIdInt) {
            case SET_IMAGE :
                SVGADynamicEntity dynamicEntity = new SVGADynamicEntity();
                dynamicEntity.setDynamicImage("https://github.com/PonyCui/resources/blob/master/svga_replace_avatar.png?raw=true", "99");
                System.out.print("=====================");
                System.out.print(args.getString(0));
                break;
            case TEST:
                root.pauseAnimation();
                break;
